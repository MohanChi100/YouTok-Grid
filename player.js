...
function showCommentList(vid) {
    $.ajax({
        // Your API endpoint to fetch comments
        ...
        success: function(data) {
            ...
            commentList.forEach(function(comment, index) {
                ...
                commentHTML += '<li class="comment-item">' +
                    ...
                    '<span class="author-name">@' + comment.author_name + '&middot;' + comment.publish_date + '</span>' +
                    // '<span class="author-name">@' + comment.author_name + '&middot;' + comment.publish_date_difference + ' days ago</span>' +
                    '</div>' +
